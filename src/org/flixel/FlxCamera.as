package org.flixel
{
	...
	public class FlxCamera extends FlxBasic
	{
		 ...
		 * Internal, used to control the "fade" special effect.
		 */
		protected var _fxFadeReset:Boolean;
		/**
		 * Internal, used to control the "fade" special effect.
		 */
		...
		override public function update():void
		{
			...
			if(target != null)
			{
				...
				else
				{
					...
					var spriteTarget:FlxSprite = target as FlxSprite; //simpleRender @author CC
					if((spriteTarget == null) || ((spriteTarget.angle == 0) && (spriteTarget.scale.x == 1) && (spriteTarget.scale.y == 1))) {
			...
			//Update the "fade" special effect, @author CC I modified this statement
			if((_fxFadeAlpha > 0.0) && (_fxFadeAlpha < 1.0))
			{
				_fxFadeAlpha += FlxG.elapsed / _fxFadeDuration;
				FlxG.flashObjects.alpha -= FlxG.elapsed / _fxFadeDuration;
				if(_fxFadeAlpha >= 1.0)
				{
					if (_fxFadeReset)
					{
						_fxFadeAlpha = 0;
						FlxG.flashObjects.alpha = 1;
					}
		 ...
		 * @param	reset		clear the fade once it's finished
		 * @param	Force		Force the effect to reset.
		 */
		public function fade(Color:uint=0xff000000, Duration:Number=1, OnComplete:Function=null, Reset:Boolean=false, Force:Boolean=false):void
		{
