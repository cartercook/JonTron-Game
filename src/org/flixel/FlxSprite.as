package org.flixel
{
	...
	public class FlxSprite extends FlxObject
	{
		...
		*/
		 ...
		 * you will need to use the FlxSpritePP class for pixel-perfect detection.
		 * WARNING: skewing sprites decreases rendering performance for this sprite by a factor of 10x!
		 * @author CC
		 */
		...
		override public function draw():void
		{
			...
			while(i < l)
			{
				...
				if(((angle == 0) || (_bakedRotation > 0)) && (scale.x == 1) && (scale.y == 1) && (skew.x == 0) && (skew.y == 0) && (blend == null))
				{	//Simple render
		...
		public function stamp(Brush:FlxSprite,X:int=0,Y:int=0):void
		{
			...
			//Simple draw
			if(((Brush.angle == 0) || (Brush._bakedRotation > 0)) && (Brush.scale.x == 1) && (Brush.scale.y == 1) && (Brush.skew.x == 0) && (Brush.skew.y == 0) && (Brush.blend == null))
			{
				_flashPoint.x = X;
				_flashPoint.y = Y;
