package org.flixel
{
	...
	public class FlxU
	{
		 ...
		 * @param	NoRepeat		ensures the last element in the list does not become the first. This was added by @author CC.
		 * 
		 * @return	The same Flash <code>Array</code> object that you passed in in the first place.
		 */
		static public function shuffle(Objects:Array,HowManyTimes:uint, NoRepeat:Boolean=false):Array
		{
			var i:uint = 0;
			var index1:uint;
			var index2:uint;
			...
			{
				index1 = Math.random()*(Objects.length-offset);
				index2 = Math.random()*(Objects.length-offset);
			...
			if (NoRepeat)
			{
				index1 = 1+Math.random()*(Objects.length-2)
				object = Objects[index1];
				Objects[index1] = Objects[Objects.length-1];
		...
		/**
		 * A function I just made up now to swap an element in an array with the one above it.
		 * Wheeee!
		 * 
		 * @author CC
		 */
		static public function swap(Objects:Array, Index:uint):Array
		{
